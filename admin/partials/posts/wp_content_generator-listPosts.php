...
$wp_content_generatorPostData = $wp_content_generatorQueryData->posts; ?>

 <h2>Bellow are all the posts generated by this plugin 
 	<?php if ( !empty($wp_content_generatorPostData) ) { ?>
	 	<span class="deleteSpan">
		 	<form action='<?php echo esc_url(admin_url('admin-post.php'))?>' method="">
			    <input type="hidden" name="action" value="wp_content_generator_deleteposts" />
...
<table id="wp_content_generatorListPostsTbl" class="stripe" style="width:100%">
	<thead>
		<tr>
			<th>#</th>
			<th>Post title</th>
	...
	<tbody>
		<?php
		if ( !empty($wp_content_generatorPostData) ) {
			$counter = 1;
			foreach ($wp_content_generatorPostData as $key => $postDatavalue){ ?>
				<tr>
					<td><?=$counter?></td>
